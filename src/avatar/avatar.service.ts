...
export class AvatarService {
    ...
    async getAvatar(username: string, size: string): Promise<any> {
        ...
        if (size === "large") selectedAvatar = user.avatars?.linkLarge ? user.avatars.linkLarge : "_default.large.webp";
            ...
            selectedAvatar = user.avatars?.linkMedium ? user.avatars.linkMedium : "_default.medium.webp";
        else if (size === "thumbnail")
            selectedAvatar = user.avatars?.linkThumbnail ? user.avatars.linkThumbnail : "_default.thumbnail.webp";
        else if (size === "original")
            selectedAvatar = user.avatars?.linkOriginal ? user.avatars.linkOriginal : "_default.original.webp";
        ...
        return {
            ...
            tag: user.avatars?.updatedAt
                ? String(new Date(user.avatars.updatedAt).getTime()) + selectedAvatar
                : selectedAvatar,
        };
    }
    ...
    async convertAvatar(avatarObject: any, avatarDbEntry: Avatar): Promise<any> {
        ...
        promises.push(
            sharpStream.clone().resize({ width: 1000 }).webp({ quality: 100 }).toFile(OutputAvatarOriginalPath)
        );
        ...
        promises.push(
            sharpStream.clone().resize({ width: 100 }).webp({ quality: 30 }).toFile(OutputAvatarthumbnailPath)
        );
