...
export class GameService {
	...
	constructor(private readonly usersService:UsersService,
		...
		async createGame(socketP1 : Socket, socketP2 : Socket, server : Namespace) {
			const gameEntry = await this.prismaService.game.create({data: {playerOneName: socketP1.data.username, playerTwoName: socketP2.data.username}})
